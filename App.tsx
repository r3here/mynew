...
const App: React.FC = () => {
  ...
  const handleSingleAiAnalyze = async (item: VaultItem) => {
    ...
    try {
      ...
      // Store AI result in aiSummary, do NOT overwrite user summary
  ...
  const handleBatchAiAnalyze = async () => {
    ...
    if (!confirm(`即将对 ${targetItems.length} 个项目进行 AI 分析。每条需等待 2 秒以防限流。确定继续吗？`)) return;
    ...
    for (let i = 0; i < targetItems.length; i++) {
      ...
      try {
        ...
        console.error(`Analysis failed for ${item.title}`, e);
        updatedItems.push(item); // Keep original if failed
      }
      ...
      <div className="container mx-auto px-4 py-6 flex gap-6">
        ...
        <main className="flex-1 min-w-0">
          <div className="mb-8 relative">
            <input type="text" className="block w-full pl-4 pr-4 py-3 bg-white dark:bg-slate-900 border border-transparent dark:border-slate-800 rounded-2xl text-gray-900 dark:text-slate-100 placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 outline-none shadow-sm" placeholder="搜索..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
